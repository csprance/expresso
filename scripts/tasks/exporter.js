...
const run = function (mode) {
  ...
  return Promise.try(() => {
    ...
    .then(() => {
      // Copy bundle files
      return cpy(["exporter/**/*.*", "CSInterface*.js"], buildPath, {
        cwd: "bundle/",
        parents: true,
      });
    })
    ...
    .then(() => {
      ...
      return new Promise((resolve, reject) => {
        ...
        let webpackConfig = require("../webpack.config.js").getConfig(
          mode,
          "exporter"
        );
        ...
        extend(webpackConfig.plugins[0].definitions, {
          VERSION: JSON.stringify(
            cepyConfig.builds["exporter"].extensions[0].version
          ),
          WEBSITE: JSON.stringify(
            cepyConfig.builds["exporter"].extensions[0].homepage
          ),
        });
