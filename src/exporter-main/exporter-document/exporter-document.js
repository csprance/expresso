...
export default CorePhotoshopDocument.extend({
  ...
  oninit: function () {
    ...
    this.observe(
      "targets.*.channels.* targets.*.filters.* targets.*.normal.*",
      this.modelUpdated,
      { init: false }
    );
  },
  ...
  getSaveFolder: function (basePath, savePath) {
    ...
    if (savePath.charAt(0) === ".") {
      ...
      if (String.isEmpty(basePath)) {
        throw new Error(
          "You need to save the document before exporting using a relative path."
        );
      }

      folder = nodePath.join(basePath, savePath);
      logger.debug(
        `Export folder is relative to document path ${folder} (${savePath})`
      );
    } else {
      folder = savePath;
    }
    ...
    if (!fs.existsSync(folder)) {
      ...
      logger.info(
        `Export folder ${folder} does not exists, trying to create it...`
      );
      ...
      if (!fs.existsSync(folder)) {
        throw new Error(
          `Invalid export path "${folder}": folder does not exists and could not be created.`
        );
      }
    }
  ...
  exportTarget: function (index) {
    if (
      SlowTask.isInProgress() ||
      index < 0 ||
      index >= this.get("targets").length
    ) {
      return Promise.resolve();
    } else {
      ...
      return Promise.try(() => {
        ...
        .then((documentPath) => {
          ...
          const options = this.getExportOptions();
          let targets = [
            JSON.parse(JSON.stringify(this.get("targets")[index])),
          ]; // Poor man copy
          this.resolveExportPaths(targets, documentPath);
          return Extension.get().imageExporter.run(targets, options);
        })
  ...
  resolveExportPaths: function (targets, documentPath) {
    ...
    let basePath = documentPath === null ? "" : nodePath.dirname(documentPath),
      documentFileName =
        documentPath === null
          ? "UnsavedDocument"
          : nodePath.basename(documentPath, nodePath.extname(documentPath));
    ...
    targets.forEach((target) => {
      var targetFolder = target.pathLocked
          ? globalTargetFolder
          : this.getSaveFolder(basePath, target.path),
        targetFile = documentFileName + target.suffix,
        targetExtension = Extension.get().settings.get("useLowerCaseExtension")
          ? target.format.toLowerCase()
          : target.format.toUpperCase();

      target.path = nodePath.join(
        targetFolder,
    ...
    return Promise.try(() => {
      ...
      .then((documentPath) => {
        ...
        var initialPath = this.get("path"),
          parsedDocumentPath =
            documentPath !== null ? nodePath.dirname(documentPath) : null;
