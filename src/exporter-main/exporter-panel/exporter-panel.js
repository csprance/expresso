  ...
  oninit: function () {
    ...
    CEP.setPanelFlyoutMenu({
      menu: [
        {
          label:
            CEP.getExtensionInfo().name + " " + Extension.get().info.version,
          enabled: false,
        },
        { label: "---" },
    ...
    CEP.addEventListener("com.adobe.csxs.events.flyoutMenuClicked", (event) => {
      if (event && event.data && event.data.menuId && event.data.menuName) {
        if (event.data.menuId === "openWebsite") {
          ...
          CEP.evalScript("Folder(params.logFolderPath).execute()", {
            logFolderPath: Extension.get().folders.logs,
          });
        } else if (event.data.menuId === "openSettings") {
          CEP.requestOpenExtension(
            RELEASE
              ? "com.expresso.exporter.settings"
              : "com.expresso.exporter.settings.debug"
          );
        }
      }
    });
    ...
    CEP.addEventListener("com.expresso.exporter.exportAll", () => {
      this.exportAll();
    });
    CEP.addEventListener("com.expresso.exporter.exportEnabled", () => {
      this.exportEnabled();
    });
  },
