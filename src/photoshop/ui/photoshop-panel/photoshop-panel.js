...
export default Ractive.extend({
  ...
  teardown: function () {
    ...
    Extension.get().photoshop.removePhotoshopEventListener(
      "imageChanged",
      this.imageChanged
    );
  },
  ...
  createDocumentModel: function (documentId) {
    throw new Error(
      "createDocumentModel should be implemented in derived classes."
    );
  },
  ...
  connectToPhotoshop: function () {
    ...
    Promise.try(() => {
      ...
      .then(() => {
        ...
        Extension.get().photoshop.onPhotoshopEvent(
          "imageChanged",
          this.imageChanged
        );
        ...
        return CEP.evalScript("app.documents.length").then((num) => {
          ...
          if (num > 0) {
            return Extension.get()
              .photoshop.getDocumentInfo(null, {
                compInfo: false,
                imageInfo: false,
                layerInfo: false,
  ...
  documentClosed: function (closedDocumentId) {
    Promise.try(() => {
      ...
      if (
        activeDocument &&
        activeDocument.get("documentId") === closedDocumentId
      ) {
        this.set("activeDocument", null);
      }
