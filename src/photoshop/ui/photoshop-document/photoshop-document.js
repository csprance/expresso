...
export default CoreBase.extend({
  ...
  save: function () {
    ...
    return Promise.try(() => {
      ...
      return CEP.evalScript(jsx, params).then(function () {
        return params.data;
      });
    })
      .then((data) => {
  ...
  imageChanged: function (event) {
    ...
    if (event.layers && event.layers.length) {
      ...
      this.layersUpdateHandle = setTimeout(
        this.fetchLayers,
        this.layersUpdateHysteresis
      );
    }
  },
  ...
  fetchLayers: function () {
    ...
    return Extension.get()
      .photoshop.getDocumentInfo(this.get("documentId"), {
        compInfo: false,
        imageInfo: false,
        layerInfo: true,
