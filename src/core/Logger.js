...
export class LogManager {
  ...
  constructor(options) {
    ...
    assertType(
      ...
      `Log folder should be a non-empty string: ${parsed.folder}`
    );
    assertType(
      !String.isEmpty(parsed.filename),
      `Log filename should be a non-empty string: ${parsed.filename}`
    );
    ...
    Object.defineProperties(this, {
       ...
       */
      filepath: {
        value: path.join(parsed.folder, parsed.filename),
        enumerable: true,
      },
  ...
  static get() {
    assert(
      singleton !== null,
      "Trying to access the Log manager before it has been initialized."
    );
    return singleton;
  }
  ...
  for (let name in levels) {
    if (RELEASE) {
        ...
        return function () {
          if (logLevel.priority <= context.level.priority) {
            ...
            const text = util.format(
              ...
              context.category,
              logName.toUpperCase(),
              util.format.apply(context, args)
            );
      ...
      if (levels[name].priority <= context.level.priority) {
        context[name] = Function.prototype.bind.call(
          window.console[name],
          window.console,
          "%c[%s] %c[%s] %c%s",
          context.css,
...
export default class Logger {
  ...
  constructor(category, level, css) {
    assertType(
      !String.isEmpty(category),
      "Log category needs to be a non-empty string."
    );
