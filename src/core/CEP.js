...
function parseMenuItemJSON(menu) {
  if (!(menu instanceof Array)) {
    throw new TypeError(
      "Menu should be an array of objects describing menu items."
    );
  }
...
export default class CEP {
  ...
  static evalScript(script, params) {
    return new Promise((resolve, reject) => {
      const evalString = `var params = ${
        typeof params === "object" ? JSON.stringify(params) : "null"
      };${script}`;
  ...
  static dispatchEvent(name, data, scope) {
    ...
    const event = new CSEvent(
      name,
      scope,
      CEP.getHostEnvironment().appName,
      CEP.getExtensionId()
    );
    event.data = data;
  ...
  static getExtensionInfo() {
    ...
    return csInterface.getExtensions([
  ...
  static registerKeyEventsInterest() {
    ...
    for (let i = 0; i < events.length; i++) {
      ...
      for (let j = 0; j < interestingKeyEvents.length; j++) {
        ...
        if (
          registered.event.keyCode === event.keyCode &&
  ...
  supports: {
    value: (function () {
      ...
      return Object.freeze({
        ...
        contextMenu:
          version.major >= 6 || (version.major >= 5 && version.minor >= 2),
